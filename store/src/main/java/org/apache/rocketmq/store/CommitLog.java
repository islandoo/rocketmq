...
import org.apache.rocketmq.store.schedule.ScheduleDelayThread;
        ...
        try {
            ...
            if (propertiesLength > 0) {
                ...
                // Timing message processing
                {
                    ...
                    if (TopicValidator.RMQ_SYS_SCHEDULE_TOPIC2.equals(topic) && t != null) {
                        int delayLevel = Integer.parseInt(t);

                        if (delayLevel > 0) {
                            tagsCode = this.defaultMessageStore.getDelayScheduleMessageService().computeDeliverTimestamp(delayLevel,
                                    storeTimestamp);
                        }
                    }
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        ...
        if (tranType == MessageSysFlag.TRANSACTION_NOT_TYPE
            ...
            if (msg.getDelayTime() > 0){
                if (msg.getDelayTime() > 2 * 60 * 60 * 1000){
                    msg.setDelayTime(2 * 60 * 60 * 1000);
                }
                msg.setDelayTimeLevel(ScheduleDelayThread.getDelayTimeLevel(msg.getDelayTime()));
                topic = TopicValidator.RMQ_SYS_SCHEDULE_TOPIC2;
                queueId = msg.getDelayTimeLevel();
